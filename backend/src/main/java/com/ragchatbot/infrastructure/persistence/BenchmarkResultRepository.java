...
public interface BenchmarkResultRepository extends JpaRepository<BenchmarkResult, UUID> {
    ...
    @Query("""
            ...
            FROM BenchmarkResult b
            WHERE b.runId = :runId
              AND b.evaluationSource = 'ragas-service:gemini'
              AND b.evaluationFallbackUsed = false
            GROUP BY b.chunkingStrategy, b.embeddingModel, b.experimentType
            ORDER BY b.chunkingStrategy, b.embeddingModel
            """)
    List<BenchmarkSummaryDto> findFinalMetricsByRunId(@org.springframework.data.repository.query.Param("runId") String runId);
}
