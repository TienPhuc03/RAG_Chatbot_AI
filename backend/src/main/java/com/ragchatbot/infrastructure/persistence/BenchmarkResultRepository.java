...
public interface BenchmarkResultRepository extends JpaRepository<BenchmarkResult, UUID> {
    ...
    @Query("SELECT new com.ragchatbot.application.dto.benchmark.BenchmarkSummaryDto(" +
              ...
              // Tổng số lần được đánh giá bằng RAGAS:
              // gồm tên cũ 'ragas' và hai provider ragas-service.
              "SUM(CASE WHEN b.evaluationSource = 'ragas' " +
              "OR b.evaluationSource LIKE 'ragas-service:%' " +
              "THEN 1 ELSE 0 END), " +
